...
@Singleton
class DynamoDBResource @Inject()(configuration: play.api.Configuration) extends StorageDriverTrait {

  // Read config
  val tableName: String = configuration.getString("aws.dynamoDB.table").get
  // Initialize client and read table
  implicit val dynamoDB: DynamoDB = dynamoClient
  val table: Table = dynamoDB.table(tableName).get

  // CRUD //

  def showAll(): Try[Seq[StorableTrait]] = Try {
  ...
  def update(data: StorableTrait): Try[StorableTrait] = {
    if(show(data.id).isSuccess) {
      ...
      Failure(new Throwable(s"Advert with id $data.id doesn't exist, update failed"))
    }
  }
  ...
  def create(data: StorableTrait): Try[StorableTrait] = {
    if(show(data.id).isFailure) {
      ...
      Failure(new Throwable(s"$data.attributes already exists"))
    }
  }
  ...
  case class Storable(_attributes: Seq[(String, Any)]) extends StorableTrait {
    ...
    val attributes: Seq[(String, Any)] = this._attributes.filter(entry => entry._1 != id)
  }
